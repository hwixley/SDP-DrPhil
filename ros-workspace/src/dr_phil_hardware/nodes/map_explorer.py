...
class MapExplorer:
    ...
    def door_pose_callback(self, door : PoseStamped):
        if self.spotted_door is not None and self.spotted_door.pose.position == door.pose.position:
            self.spotted_door = None
        else:
    ...
    def spin(self):
        if (self.area_explored/self.total_area)>=self.STOPPING_THRESHOLD:
            ...
            #Start with generating targets that are within 1m range
            distance_threshold = 1
            goal = explore.generate_random_target(distance_threshold)
            counter = 0
            while (not explore.is_location_available(self.robot_pose, goal) and self.was_explored(goal)):
                goal = explore.generate_random_target(distance_threshold)
                counter+=1
    ...
    def mark_all_doors(self, doors):
        markers = []
        id = 0
        for door in doors:
            marker = Marker()
            marker.header.frame_id = "map"
            marker.header.stamp =  rp
    ...
    def find_doors_nearby(self):
        ...
        while (not explore.angles_close(startingYaw, self.yaw, tolerance=3) \
            ...
            #already_found = False
            if self.spotted_door is not None:
                positions = [doors.pose.position for doors in self.door_locations]
                if self.spotted_door.pose.position not in positions:
                    self.door_locations.append(self.spotted_door)
                    print(self.spotted_door)
