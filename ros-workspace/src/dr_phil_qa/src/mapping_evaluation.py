...
class MapEvaluator(object):
    ...
    def __init__(self,measured_topic,truth_topic,dump_scan,scan_param_namespace,dump_robot_velocity,odom_topic,store_explore_lite,explore_lite_namespace,run_periodically):
        ...
        self.tp_pub = rospy.Publisher(u"~tp_map",OccupancyGrid,queue_size=1)
        self.fp_pub = rospy.Publisher(u"~fp_map",OccupancyGrid,queue_size=1)
        self.tn_pub = rospy.Publisher(u"~tn_map",OccupancyGrid,queue_size=1)
        self.fn_pub = rospy.Publisher(u"~fn_map",OccupancyGrid,queue_size=1)
        self.combined_pub = rospy.Publisher(u"~combined_map",OccupancyGrid,queue_size=1)
        ...
        self.data_home_path = os.path.join(os.path.dirname(os.path.realpath(__file__)),u"..",u"data")

        self.evaluate_trigger = rospy.Service(u"~evaluate",EvaluateMaps,self.evaluate)
            ...
            if m is not None:
                ...
                combined_map.data = [ x if y == GRID_VALUES.UNKNOWN else 
                                        (y if x == GRID_VALUES.UNKNOWN else 
                                            GRID_VALUES.UNKNOWN )for x,y in izip(combined_map.data,m.data)]
    ...
    def evaluate(self,args):
        u""" performs the evaluation between ground truth and measured maps """
        ...
        with self.data_guard: # don't want maps to change mid calculation
            ...
            areas[u"free_area_mesured_over_truth"] = free_area_measured / free_area_truth
            areas[u"occupied_area_mesured_over_truth"] = occupied_area_measured / occupied_area_truth
            
            areas[u"total_area_mesured_over_truth"] = (occupied_area_measured + free_area_measured) / (occupied_area_truth + free_area_truth)
            ...
            if self.dump_twist:
                rospy.logerr(unicode(self.dump_twist))
                self.linear_vels.append(abs(self.odom.twist.twist.linear.x))
                self.ang_vels.append(abs(self.odom.twist.twist.angular.z))

                quality[u"average_ang_vel"] = sum(self.ang_vels)/ len(self.ang_vels) 
                quality[u"average_lin_vel"] = sum(self.linear_vels)/ len(self.linear_vels) 

        
        quality[u"time_elapsed"] = (rospy.rostime.get_time() - self.time_start) / 60

        raw[u"true_positives"] = tp_num
        raw[u"true_negatives"] = tn_num
        raw[u"false_positives"] = fp_num
        raw[u"false_negatives"] = fn_num

        rates[u"true_positive_rate"] = tp_num / (tp_num + fn_num)
        rates[u"false_positive_rate"] = fp_num / (fp_num + tn_num)
        rates[u"overall_error"] = (fp_num + fn_num) / (tp_num + fn_num + fp_num + tn_num)

        metrics[u"rates"] = rates
        metrics[u"raw"] = raw
        metrics[u"areas"] = areas 
        metrics[u"quality"] = quality
        values[u"values"] = metrics
        ...
        file_path = os.path.join(self.data_home_path,args.filename + u".yml")

        if os.path.isfile(file_path) and ((not self.run_periodically) or self.run_periodically and self.last_args == None):
            rospy.logerr(u"file already exists at: {0}. Aborting".format(file_path))
            return None

        with open(file_path, u'w') as f:  # You will need 'wb' mode in Python 2.x
            yaml.dump(values,f,default_flow_style=False)

        if self.dump_scan:
            scan_params = rospy.get_param(self.scan_param_namespace)
            if scan_params is not None:
                with open(os.path.join(os.path.dirname(file_path),args.filename+u"-scan.yml"), u'w') as f:
        ...
        if self.store_explore_lite:
            ...
            if explore_params is not None:
                with open(os.path.join(os.path.dirname(file_path),args.filename+u"-explore.yml"), u'w') as f:
    ...
    def apply_count_over_map(self,m,GT,counter, map_filter = lambda x: x):
        ...
        if m.info.origin != GT.info.origin:
            raise ValueError(u"Maps do not have the same origin")
        elif m.info.resolution != GT.info.resolution:
            raise ValueError(u"Maps do not have the same resolution")
        ...
        for y in xrange(GT.info.height):
            for x in xrange(GT.info.width):
        ...
        if GT.info.height < m.info.height:
            ...
            for y in xrange(GT.info.height,m.info.height):
                for x in xrange(m.info.width):
        ...
        if GT.info.width < m.info.width:
            ...
            for y in xrange(m.info.height):
                for x in xrange(GT.info.width,m.info.width):
                    # element on m map 
                    m_elem = m.data[self.map_index_from_2d(y,x,m.info.width)]
    ...
    def calculate_area(self,map,include_condition = lambda x: x == GRID_VALUES.FREE):
        """ calculates total number of cells with prior satisfying the include_condition """
        area = sum([1 for x in imap.data if include_condition(x)])
        
        return float(area)


if __name__ == u"__main__":
    rospy.init_node(u"map_evaluator")

    measured_topic = rospy.get_param(u"~measured_topic",u"/map_measured")
    truth_topic = rospy.get_param(u"~truth_topic",u"/map_truth")
    dump_scan = rospy.get_param(u"~store_scan_params",False)
    scan_param_namespace = rospy.get_param(u"~scan_param_namespace",u"/slam_gmapping")
    run_periodically = rospy.get_param(u"~run_periodically",False)
    store_velocity = rospy.get_param(u"~store_robot_velocity",False)
    odom_topic = rospy.get_param(u"~odom_topic",u"/odom")
    store_explore_lite = rospy.get_param(u"~store_explore_lite",False)
    explore_lite_namespace = rospy.get_param(u"~explore_lite_namespace",u"/explore")
    ...
    if run_periodically:
        rospy.loginfo(u"run_periodically setting is on, after first service call will repeatedely evaluate")
